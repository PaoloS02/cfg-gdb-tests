...
#define UNBALANCED_OUTPUT_FILE "unbalanced_test_results.txt"
...
int main(int argc, char **argv) {
	...
	if(argc > 1){
		if(strcmp(argv[1], "unbalanced") == 0)
			unbalanced = true;
	}
	
	//enum test_instances 
	FILE* fout = fopen(OUTPUT_FILE,"a");
	FILE* fout_unb = fopen(UNBALANCED_OUTPUT_FILE, "a");
	...
	for(test = 1; test <= 12; test++) {
		for(inst = 0; inst < inst_num[test-1]; inst++) {
			...
			*/	
			...
			if(unbalanced)
					parse_test(test, inst+1, fout_unb);
			else
